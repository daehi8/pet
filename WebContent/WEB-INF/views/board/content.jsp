...
<%@ include file="../nav.jsp" %>
	...
	<input class="btn btn-dark" style="float: left;" type="button" value="목록" onclick="window.location='/pet/board/list.do'" />
	<c:if test="${sessionScope.doctorMail == dto.email || sessionScope.member.approval_status == 2}">
	<input class="btn btn-outline-dark" style="float: right; margin-right:10px" type="button" value="삭제" onclick="window.location='/pet/board/deleteForm.do?num=${dto.num}'" />		
	<input class="btn btn-dark" style="float: right; margin-right:2px" type="button" value="수정" onclick="window.location='/pet/board/updateForm.do?num=${dto.num}'" />
	</c:if>
